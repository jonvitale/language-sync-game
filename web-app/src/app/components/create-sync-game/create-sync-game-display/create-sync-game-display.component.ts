...
export class CreateSyncGameDisplayComponent implements OnInit {
  ...
  createNewSync() {
  	if (this.pApp != null && this.transcriptService.currentTranscript.length > 0) {
  		this.index = 0;
  		console.log(this.transcriptService.currentTranscript);
  ...
  advanceSync() {
  	if (this.isPlaying && this.index < this.transcriptService.currentTranscript.length) {
  		this.transcriptService.currentTranscript.setTimeAt(this.index, (new Date()).getTime() - this.startTime);
  		this.shiftPTexts();
	  }
  }
  ...
  advanceCurrentSync() {
  	if (this.isPlaying && this.index < this.transcriptService.currentTranscript.length) {
  		let time = this.transcriptService.currentTranscript.getTimeAt(this.index) - (this.index > 0 ? this.transcriptService.currentTranscript.getTimeAt(this.index-1) : 0);
  ...
  shiftPTexts() {
  	if (this.pApp != null && this.transcriptService.currentTranscript.length > 0) {
  		...
  		pText.text = this.transcriptService.currentTranscript.getWordAt(index);
  		console.log("new word:", this.transcriptService.currentTranscript.getWordAt(index), "at index", index);
  		this.pTexts.push(pText);
  ...
  defineCurrentPTexts(): void {
  	if (this.pApp != null && this.transcriptService.currentTranscript.length > 0) {
  		...
  		if (this.index - 1 >= 0) {
  			curX = this.width/2 - 20;
  			for (let i = this.pTexts.length / 2 - 1; i >= 0; i--){
	  			let word: string = this.transcriptService.currentTranscript.getWordAt(this.index - (this.pTexts.length / 2 - i));
  		...
  		if (this.index < this.pTexts.length) {
  			curX = this.width/2 + 20;
  			for (let i = this.pTexts.length / 2; i < this.pTexts.length; i++){
	  			let word: string = this.transcriptService.currentTranscript.getWordAt(this.index + (i - this.pTexts.length / 2));
	  			this.pTexts[i].text = word;
	  			this.pTexts[i].x = curX; 
	  			curX += this.pTexts[i].width + this.wordSpacing;
	  			if (i == this.pTexts.length / 2){
